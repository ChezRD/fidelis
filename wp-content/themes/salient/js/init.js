...
/***************** Superfish ******************/
	...
	/*$('header#top nav > ul > li').mouseleave(function(){
		if(!$(this).hasClass('megamenu') && !$('#header-outer').hasClass('transparent')){
...
/***************** Full Width Section ******************/
	function fullWidthSections(){
		...
		if($('#boxed').length == 1){
			...
			if($(window).width() <= parseInt($('.container').css('max-width'))) { 
				var $windowWidth = parseInt($('.container').css('max-width'));
			...
			$justOutOfSight = Math.ceil( (($windowWidth + $extResponsivePadding + $scrollBar - parseInt($('.container').css('max-width'))) / 2) )
		}
		...
		//standard carousel
			...
			$(this).find('> .span_12 > .col').each(function(){
         	...
         	if($(this).hasClass('vertically-align-columns') && window.innerWidth > 1000){
				...
				//child columns
				/*$(this).find('> .span_12 > .col .wpb_row > .col').each(function(){
					
					$columnInnerHeight = $(this).find('.wpb_wrapper').height();
					var $topPadding = parseInt($(this).find('> .col').css('padding-top'));
					var $marginCalc = (Math.floor(($(this).height()-$topPadding*2)/2)-($columnInnerHeight/2));
					if($marginCalc <= 0) $marginCalc = 0;
					if($topPadding > 0 &&  window.innerWidth > 1000){
						$(this).find('.wpb_wrapper').css('margin-top',$marginCalc);
						$(this).find('.wpb_wrapper').css('margin-bottom',$marginCalc);
					}
				});	*/
...
if($('#page-header-bg[data-parallax="1"]').length > 0) {
	...
	function pageHeaderInit(){
	 //transparent
	  if($('#header-outer[data-transparent-header="true"]').length > 0) {	
	     $('#page-header-bg[data-parallax="1"]').css({'top': extraHeight+secondaryHeader+wooCommerceHeader });
	  } else {
